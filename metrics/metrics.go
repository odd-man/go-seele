...
func init() {
	// MetricsLogs = slog.GetLogger("metrics", common.PrintLog)
	MetricsLogs = slog.GetLogger("metrics", true)
	Enabled = true
}
...
func CollectProcessMetrics(refresh time.Duration) {
	...
	if err := ReadDiskStats(diskstats[0]); err == nil {
		...
		MetricsLogs.Debug("Failed to read disk metrics", "err", err)
	}
